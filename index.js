...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Hanan-Algifri\n' // GANTI NAMA LU
            + 'ORG: SENSEI;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6288289824963 :+62 882-8982-4963\n' // GANTI NOMOR KAU
            + 'END:VCARD'
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SCAN QR MU <POWERED BY Abdi-Bot>'))
})

	baby.on('credentials-updated', () => {
		fs.writeFileSync('./Miku.json', JSON.stringify(baby.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ABDI-BOT Loading...')
	})
	fs.existsSync('./Miku.json') && baby.loadAuthInfo('./Miku.json')
	baby.on('connecting', () => {
		start('2', 'ABDI-BOT Connecting...')
	})
	baby.on('open', () => {
		success('2', 'ABDI-BOT Connected')
	})
...
*/
                ...
                addKoinUser('6282335452441@s.whatsapp.net', fee)
...
cloud = body.slice(11)
reply('「❗」Bentar Bro Gw Terbang dulu yakan')
